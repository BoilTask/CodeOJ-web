			...
			saveContent: function() {
				if (this.formValidate.discuss_id == '') {
					...
					if (this.content.length < 1) {
						...
						axios
							.post(this.$store.state.API_ROOT + 'discuss/'+this.formValidate.discuss_id+'/reply/add', params)
							.then(response => {
								if (response.data.data.is_ok) {
									this.$Message.success('保存成功！');
									this.$router.push('/discuss/'+this.formValidate.discuss_id+'/reply/'+response.data.data.reply_id+'/edit')
								} else
